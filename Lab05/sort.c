...
int main(void) {
    ...
    printf("\n\nWelcome to kcamaner's sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
...
ListItem *SelectionSort(ListItem* list) {
    ...
    while (first->nextItem) {
        ListItem *comp = first->nextItem;
        while (comp) {
            if (strcmp(first->data, comp->data) > 0) {
                LinkedListSwapData(first, comp);
            }
            comp = comp->nextItem;
        }
...
ListItem *InsertionSort(ListItem* list) {
    ...
    ListItem *first = LinkedListGetLast(list);


    while (first->previousItem) {

        ListItem *unsorted = first -> previousItem;

        if (strcmp(unsorted->data, first->data) < 0) {
            first = unsorted;
        } else {

            scan = first;
            while (scan->nextItem) {
                if (strcmp(scan->nextItem->data, unsorted->data) > 0) {
                    break;
                } else {
                    scan = scan->nextItem;
                }
